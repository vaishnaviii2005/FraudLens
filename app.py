...
# Premium Custom CSS for Dark Mode
st.markdown(
    """
    <style>
    /* Metric styling */
...
def load_model_artifacts():
    ...
    # Optional metrics and importances
    try:
...
def predict_dataframe(df_input):
    ...
    for col in model_columns:
        if col.startswith("policy_type_"):
            ...
            if "policy_type" in df_cleaned.columns:
                encoded_df[col] = (df_cleaned["policy_type"] == val).astype(int)
            else:
                encoded_df[col] = 0
        elif col.startswith("claim_history_"):
            val = col.replace("claim_history_", "")
            if "claim_history" in df_cleaned.columns:
                encoded_df[col] = (df_cleaned["claim_history"] == val).astype(int)
            else:
                encoded_df[col] = 0
...
st.markdown("<p class='sub-header'>A sophisticated machine learning engine monitoring risk, analyzing patterns, and predicting claims fraud.</p>", unsafe_allow_html=True)
    ...
    if submit_btn:
        ...
        with m_col1:
            st.markdown(
                ...
                <div class="metric-container">
                    <p style="margin:0; color:#9ca3af; font-size:0.9rem; text-transform:uppercase; letter-spacing:0.05em;">FRAUD RISK SCORE</p>
                    <p style="margin:0; font-size:2.8rem; font-weight:800; color:{'#ef4444' if classification == 'High Risk' else '#10b981'};">{fraud_percent}%</p>
                </div>
                """, 
                unsafe_allow_html=True
            )
        with m_col2:
            st.markdown(
                f"""
                <div class="metric-container">
                    <p style="margin:0; color:#9ca3af; font-size:0.9rem; text-transform:uppercase; letter-spacing:0.05em;">RISK CLASSIFICATION</p>
                    <p style="margin:0; font-size:2.8rem; font-weight:800; color:{'#ef4444' if classification == 'High Risk' else '#10b981'};">{classification}</p>
                </div>
                """, 
                unsafe_allow_html=True
            )
...
with tab3:
    ...
    with col_metrics:
        ...
        if feature_importances is not None:
            ...
            selected_model_name = st.selectbox("Select Model to Inspect Importances", list(feature_importances.keys()))
            imp_dict = feature_importances[selected_model_name]
    ...
    with eda_col1:
        dist_path = os.path.join(PLOTS_DIR, "claim_amount_distribution.png")
        if os.path.exists(dist_path):
            st.image(dist_path, caption="Distribution of Claim Amounts", use_container_width=True)
            
    with eda_col2:
        fraud_path = os.path.join(PLOTS_DIR, "fraud_vs_nonfraud.png")
        if os.path.exists(fraud_path):
            st.image(fraud_path, caption="Target Distribution (Fraud vs Normal)", use_container_width=True)
            
    with eda_col3:
        age_path = os.path.join(PLOTS_DIR, "age_distribution.png")
        if os.path.exists(age_path):
            st.image(age_path, caption="Age Distribution of Claimants", use_container_width=True)
